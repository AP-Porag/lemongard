...
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { UserCheck, Search, Plus, Eye, Pencil, Trash2, Lock } from 'lucide-react';
...
export default function Dashboard({
    ...
    // ✅ Search access gate: any one of the three flags being true unlocks search
    const canSearch =
        user?.has_full_access === true ||
        user?.tier_2_full_access === true ||
        user?.tier_1_view_only === true;
    ...
    const handleSearch = () => {
        if (!canSearch) return; // ✅ blocked when no access flag is true
    ...
    const inputClass = (field: string) =>
        `w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400 disabled:cursor-not-allowed disabled:bg-slate-100 ${searchErrors[field] ? 'border-red-500 focus:ring-red-500' : 'border-gray-300 focus:border-yellow-400'
        }`;
            ...
            <div className="min-h-screen bg-slate-50/50 p-4 md:p-8">
                <div className="mx-auto max-w-7xl space-y-8">
                    ...
                    <Card className="border-none shadow-sm ring-1 ring-slate-200">
                        ...
                        <CardContent className="p-6">
                            {/* ✅ Upgrade message when no access flag is true */}
                            {!canSearch && (
                                <div className="mb-4 flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-700">
                            ...
                            <div className="flex flex-col gap-4 md:flex-row md:items-end">
                                <div className="relative flex-1">
                                    <label className="mb-1 block text-xs font-medium text-gray-600">Last Name <span className="text-red-500">*</span></label>
                                    <input name="last_name" value={searchForm.last_name} onChange={handleSearchChange} onKeyDown={handleKeyDown} disabled={!canSearch} className={inputClass('last_name')} placeholder="Last name" />
                                    {searchErrors.last_name && <p className="absolute left-0 top-full mt-1 text-xs text-red-500">{searchErrors.last_name}</p>}
                                </div>
                                <div className="relative flex-1">
                                    <label className="mb-1 block text-xs font-medium text-gray-600">First Name <span className="text-red-500">*</span></label>
                                ...
                                <div className="relative flex-1">
                                    <label className="mb-1 block text-xs font-medium text-gray-600">Phone <span className="text-red-500">*</span></label>
                                    <input name="phone" value={searchForm.phone} onChange={handleSearchChange} onKeyDown={handleKeyDown} maxLength={12} disabled={!canSearch} className={inputClass('phone')} placeholder="XXX-XXX-XXXX" />
                                    {searchErrors.phone && <p className="absolute left-0 top-full mt-1 text-xs text-red-500">{searchErrors.phone}</p>}
                                </div>
